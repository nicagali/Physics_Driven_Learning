...
def training_epoch(G):
    ...
    voltages_change_file = open(f"{PATH_TO_DIR}data/voltages_simple_change_continuous_conductances.txt", "w") 
    voltages_change_file.write(f"{desired_output[0]}\n")

    conductances_change_file = open(f"{PATH_TO_DIR}data/conductances_change.txt", "w") 

    mse_file = open(f"{PATH_TO_DIR}data/mse_continuous_conductances.txt", "w")

    odeint_solver.euler_forward_solver('square', conductance_numb='g1')
    data = np.loadtxt(f"{PATH_TO_DIR_ION}data/solution_euler.txt", unpack=True)
    g_solution1 = data[1]*g_0*(10**12)
